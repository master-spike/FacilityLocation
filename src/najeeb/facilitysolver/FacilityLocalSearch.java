...
import najeeb.localsearch.SimulatedAnnealing;

public class FacilityLocalSearch extends SimulatedAnnealing {

	public FacilityLocalSearch(Customer[] customers, Facility[] facilities) {
	...
	class ReassignCustomer extends LocalMove {
		...
		public boolean isLegal() {
			if (customers[customer].demand > facilities[facility].cap - used_caps[facility]) {
				return false;
			} else
				return true;
		}
	...
	class LocalMovePriorityQueue {

		public LocalMovePriorityQueue(int num) {
			heap = new LocalMove[num];
		}

		void insert(LocalMove lm) {
			heap[firstspace] = lm;
			...
			while (heap[(position - 1) / 2].calcNetChange() < heap[position].calcNetChange()) {
				LocalMove swap = heap[(position - 1) / 2];
		...
		LocalMove remove(int index) {
			LocalMove toReturn = heap[index];
		...
		// some variables to speed up insertion
		int[] spaces;
		int firstspace;
		int num_spaces;
	...
	protected boolean isInTerminationCondition() {
	...
	protected double tempFunction(int steps) {
		return Main.radius * Math.exp(-steps / (double) (20 * customers.length * facilities.length));
	}

	@Override
	protected LocalMove selectNextMove() {
		...
		for (int i = 0; i < facilities.length; i++) {
			if (facilities[i].cap < used_caps[i]
					&& facilities[i].cap - used_caps[i] <= facilities[max_viol_f].cap - used_caps[max_viol_f]) {
				max_viol_f = i;
				any_violations = true;
			}
		}
	...
	protected void genInitState() {
		...
		int[] facil_order = Main.genRandomPermutation(facilities.length);
		int x = 0;
		...
		for (int i = 0; i < customers.length; i++) {
			while (facilities[facil_order[x]].cap
					- used_caps[facil_order[x]] < customers[Main.customer_order_demand[i]].demand) {
				x++;

			}
			assignment[Main.customer_order_demand[i]] = facil_order[x];
			used_caps[facil_order[x]] += customers[Main.customer_order_demand[i]].demand;
